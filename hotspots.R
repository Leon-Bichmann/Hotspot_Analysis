...
for (protid in unique(df$Single.Proteins)){
  ...
  if (exclusive_switch){
    ...
    write.csv(sums, file = paste0(outdir,"/only_malign/",protid,"_coverage.csv"))
    ...
    write.csv(sums, file = paste0(outdir,"/only_benign/",protid,"_coverage.csv"))
    ...
    write.csv(sums, file = paste0(outdir,"/all/",protid,"_coverage.csv"))
